...
const int desiredDistanceBetweenTwoBlocks = 2 * BLOCK_RADIUS;
...
//	bool keys[4] = {false, false, false, false};
	...
	ALLEGRO_COLOR blackColor = al_map_rgba(0, 0, 0, 100);
	ALLEGRO_COLOR whiteColor = al_map_rgba(255, 255, 255, 100);
	...
	while(gameLoop == true)
	{
		...
		if(ev.type == ALLEGRO_EVENT_KEY_DOWN)
		{
			...
			{
				std :: cout << "before calling increaseBlockNumber function\n";
				increaseBlockNumber(block);
			}
			else if(ev.keyboard.keycode == ALLEGRO_KEY_MINUS)
			{
				std :: cout << "before calling decreaseBlockNumber function\n";
		...
		if(ev.type == ALLEGRO_EVENT_TIMER)
		{
			...
			*/
			...
			/*
			// update part 2 version 2.0
	
			// block[1] is going up
			if(block[1].xVelocity == 0 && block[1].yVelocity<0 )
			{
				if(block[1].y <= finalPreviousState.y)
				{
					updateBlockDirection(block);	
				}
			}
			// block[1] is going down
			else if(block[1].xVelocity == 0 && block[1].yVelocity>0 )
			{
				if(block[1].y >= finalPreviousState.y)
				{
					updateBlockDirection(block);	
			...
			// block[1] is going left
			else if(block[1].yVelocity == 0 && block[1].xVelocity<0 )
			{
				if(block[1].x <= finalPreviousState.x)
				{
					updateBlockDirection(block);	
				}
			}
			// block[1] is going right
			else if(block[1].yVelocity == 0 && block[1].xVelocity>0 )	
			{
				if(block[1].x >= finalPreviousState.x)
				{
			...
			*/
			...
			//update part 2 version 3.0 
			// calculating the distance between 2 points and 
			// comparing it with 2*radius

			if(((block[0].x-finalPreviousState.x)*(block[0].x-finalPreviousState.x)+
				(block[0].y-finalPreviousState.y)*(block[0].y-finalPreviousState.y))
				 > (desiredDistanceBetweenTwoBlocks*desiredDistanceBetweenTwoBlocks))
			{
				updateBlockDirection(block);
				finalPreviousState = block[0];
			}
		...
		if(redraw && al_is_event_queue_empty(event_queue))
		{
			...
			al_draw_textf(font18, al_map_rgb(255,0,255), WIDTH-10, 70, ALLEGRO_ALIGN_RIGHT, "fps_x: %i", finalPreviousState.x);
			al_draw_textf(font18, al_map_rgb(255,0,255), WIDTH-10, 90, ALLEGRO_ALIGN_RIGHT, "fps_y: %i", finalPreviousState.y);

			for(int i=0; i<= NO_OF_ALIVE_BLOCK; i++)
			{
				al_draw_filled_circle(block[i].x, block[i].y, BLOCK_RADIUS, al_map_rgba(255, 255, 255, 20));	
			}
			al_flip_display();
...
void updateBlockDirection(SnakeBlock block[])
{
	...
	{
		std :: cout << "inside Direction change function\n";
		std :: cout << "i: " << i << "\n";
...
void increaseBlockNumber(SnakeBlock block[])
{
	if(NO_OF_ALIVE_BLOCK < MAX_BLOCK_NO)
	{
		std :: cout << "inside calling increaseBlockNumber function\n";
		NO_OF_ALIVE_BLOCK++;
		...
		std :: cout << "pb_x: "<< block[i-1].x << "pb_y: " << block[i-1].y << "\n";
		std :: cout << "nb_x: "<< block[i].x << "nb_y: " << block[i].y << "\n";
	}
}
void decreaseBlockNumber(SnakeBlock block[])
{
	...
	{
		std :: cout << "inside calling decreaseBlockNumber function\n";
		int i = NO_OF_ALIVE_BLOCK;
		block[i].live = false;
		block[i].x = 0;
		block[i].y = 0;
